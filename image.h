...
public:
	...
	/*
	typedef struct tagRotateInfo//회전
	{
		DWORD resID;		//리소스 불러올때 사용할 ID값
		HDC hMemDC;
		HBITMAP hBit;
...
public:
	...
	HRESULT init(const char* fileName, int width, int height,
		BOOL trans = FALSE, BOOL rotate = FALSE, COLORREF transColor = FALSE);
	
	//프레임 이미지 용 초기화 함수
	HRESULT init(const char* fileName, float x, float y,
		int width, int height,
		int frameX, int frameY, BOOL trans = FALSE, BOOL rotate = FALSE, COLORREF transColor = FALSE);
